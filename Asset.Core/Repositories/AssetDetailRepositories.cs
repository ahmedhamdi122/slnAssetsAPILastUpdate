...
namespace Asset.Core.Repositories
{
    public class AssetDetailRepositories : IAssetDetailRepository
    {
        ...
        public IndexAssetDetailVM ListHospitalAssets(SortAndFilterVM data, int first, int rows)
        {
            ...
            if (!string.IsNullOrEmpty(data.sortFiled))
            {
                ...
                switch (data.sortFiled)
                {
                    case "Barcode":
                    case "الباركود":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Barcode) : query.OrderByDescending(x => x.Barcode);
                        break;
                    case "Serial":
                    case "السيريال":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.SerialNumber) : query.OrderByDescending(x => x.SerialNumber);
                        break;
                    case "Model":
                    case "الموديل":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.MasterAsset.ModelNumber) : query.OrderByDescending(x => x.Barcode);
                        break;
                    case "Name":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.MasterAsset.Name) : query.OrderByDescending(x => x.MasterAsset.Name);
                        break;

                    case "الاسم":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.MasterAsset.NameAr) : query.OrderByDescending(x => x.MasterAsset.NameAr);
                        break;
                    case "Hospital":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.Name) : query.OrderByDescending(x => x.Hospital.Name);
                        break;

                    case "المستشفى":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.NameAr) : query.OrderByDescending(x => x.Hospital.NameAr);
                        break;
                    case "Brands":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.MasterAsset.brand.Name) : query.OrderByDescending(x => x.MasterAsset.brand.Name);
                        break;
                    case "الماركات":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.MasterAsset.brand.Name) : query.OrderByDescending(x => x.MasterAsset.brand.Name);
                        break;
                    case "Department":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Department.Name) : query.OrderByDescending(x => x.Department.Name);
                        break;
                    case "القسم":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Department.NameAr) : query.OrderByDescending(x => x.Department.NameAr);
                        break;
                    case "Governorate":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.Governorate.Name) : query.OrderByDescending(x => x.Hospital.Governorate.Name);
                        break;

                    case "المحافظة":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.Governorate.NameAr) : query.OrderByDescending(x => x.Hospital.Governorate.NameAr);
                        break;
                    case "City":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.City.Name) : query.OrderByDescending(x => x.Hospital.City.Name);
                        break;

                    case "المدينة":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.City.NameAr) : query.OrderByDescending(x => x.Hospital.City.NameAr);
                        break;
                    case "Organization":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.Organization.Name) : query.OrderByDescending(x => x.Hospital.Organization.Name);
                        break;

                    case "الهيئة":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.Organization.NameAr) : query.OrderByDescending(x => x.Hospital.Organization.NameAr);
                        break;
                    case "SubOrganization":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.SubOrganization.Name) : query.OrderByDescending(x => x.Hospital.SubOrganization.Name);
                        break;
                    case "هيئة فرعية":
                        query = data.sortOrder == 1 ? query.OrderBy(x => x.Hospital.SubOrganization.NameAr) : query.OrderByDescending(x => x.Hospital.SubOrganization.NameAr);
                        break;
                    default:
            ...
            foreach (var item in lstResults)
            {
                ...
                getDataobj.AssetName = item.MasterAsset.Name;
                getDataobj.AssetNameAr = item.MasterAsset.NameAr;
                getDataobj.GovernorateId = item.Hospital?.Governorate.Id??null;
                getDataobj.GovernorateName = item.Hospital!=null ? item.Hospital.Governorate.Name:"";
                getDataobj.GovernorateNameAr = item.Hospital?.Governorate.NameAr??"";
                getDataobj.CityId = item.Hospital?.City?.Id ?? 0;
                getDataobj.CityName = item.Hospital?.City?.Name ?? "";
                getDataobj.CityNameAr = item.Hospital?.City?.NameAr ?? "";
                getDataobj.OrganizationId = item.Hospital?.Organization?.Id ?? 0;
                getDataobj.OrgName = item.Hospital?.Organization?.Name ?? "";
                getDataobj.OrgNameAr = item.Hospital?.Organization?.NameAr ?? "";
                getDataobj.SubOrgName = item.Hospital?.SubOrganization?.Name ?? "";
                getDataobj.SubOrgNameAr = item.Hospital?.SubOrganization?.NameAr ?? "";
                getDataobj.SubOrganizationId = item.Hospital?.SubOrganization?.Id ?? 0;
                getDataobj.BrandId = item.MasterAsset.brand != null ? item.MasterAsset.brand.Id : 0;
                getDataobj.BrandName = item.MasterAsset.brand != null ? item.MasterAsset.brand.Name : "";
                getDataobj.BrandNameAr = item.MasterAsset.brand != null ? item.MasterAsset.brand.NameAr : "";
                getDataobj.SupplierId = item.Supplier != null ? item.Supplier.Id : 0;
